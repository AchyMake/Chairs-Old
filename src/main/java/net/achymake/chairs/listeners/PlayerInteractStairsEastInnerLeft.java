...
public class PlayerInteractStairsEastInnerLeft implements Listener {
    private Database getDatabase() {
        return Chairs.getDatabase();
    }
    public PlayerInteractStairsEastInnerLeft(Chairs plugin) {
        plugin.getServer().getPluginManager().registerEvents(this, plugin);
    }
    @EventHandler(priority = EventPriority.NORMAL)
    public void onStairsEastInnerLeft(PlayerInteractEvent event) {
        ...
        if (!getDatabase().isEast(getDatabase().getStair(event.getClickedBlock())))return;
