...
public class ChairsClickStairsSouthInnerRight implements Listener {
    public ChairsClickStairsSouthInnerRight(Chairs plugin) {
        plugin.getServer().getPluginManager().registerEvents(this, plugin);
    }
    @EventHandler(priority = EventPriority.NORMAL)
    public void onClickStairsSouthInnerRight(PlayerInteractEvent event) {
        ...
        if (!((Stairs) event.getClickedBlock().getBlockData()).getShape().equals(Stairs.Shape.INNER_RIGHT))return;
        ...
        ChairsSettings.sitStairsSouthInnerRight(event.getPlayer(), event.getClickedBlock().getLocation().add(0.5, -0.4, 0.5));
    }
}