...
public class PlayerInteractStairsSouth implements Listener {
    private Database getDatabase() {
        return Chairs.getDatabase();
    }
    public PlayerInteractStairsSouth(Chairs plugin) {
    ...
    public void onStairsSouth(PlayerInteractEvent event) {
        ...
        if (!getDatabase().isStraight(getDatabase().getStair(event.getClickedBlock())))return;
